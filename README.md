...
- Регистрация по номеру телефона + OTP, username, профиль, аватар
...
Регистрация в dev: любой E.164 номер. OTP выдаётся **только** в ответе
`POST /v1/auth/request-otp` при `OTP_DEV_REVEAL=true` (в production это
выключено и OTP уходит в SMS).
...
- Абсолютной анонимности (нужен номер телефона, сервер видит граф доставки)
