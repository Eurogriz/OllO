...
Регистрация: клиент создаёт **account** Ed25519 (адрес) и отдельную
device-пару X3DH, подписывает челлендж `POST /v1/auth/challenge`
ключом аккаунта и вызывает `POST /v1/auth/register-key`. Приватные
ключи на сервер не уходят. Restore и второе устройство выпускают новый
device identity. Второе устройство можно привязать account-only файлом
или `ollo:link:v1:` (без истории и без device IK). Сразу после создания
аккаунта нужно скачать зашифрованную резервную копию — без неё
потерянный телефон невосстановим. OTP по телефону не копирует device IK
на `account_ed25519`.
