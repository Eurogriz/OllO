...
| Group membership inject | M | Extra recipient | Signed membership (`signMembership` / `planTrustedMembers` / `planFanoutRecipients`); invite-join stays pending | Server can still **drop** members (availability). Stolen admin can sign a rogue roster |
