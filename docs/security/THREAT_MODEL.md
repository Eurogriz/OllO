...
|---|---|---|---|---|
| Stolen account Ed25519 | M | New device on the same account | Key never leaves the device; registration lock; new-device warning; OTP cannot mint the account from a device IK; account-only link (`ollo:link:v1:`) carries no history or device identity | Backup / link passphrase leak; malware |
...
| Group membership inject | M | Extra recipient | Signed membership; prior-admin; confirm; rejected hashes; first roster from another device is confirm not TOFU; sender-key ingest only for trusted (`hold` until confirm); own-other-device pending quarantines that device's sender keys; revoke/decline prunes and refuses re-ingest (`droppedDevices`); remaining admin bumps sender-key epoch after revoke; non-admin replaces its own chain; hostile decline rotates too; E2EE `device_drop` after revoke (directory must show target gone); after epoch N, other epochs are not decrypted or ingested; `planSenderKeyShare` sends the current SKDM to a newly linked member device; a PreKey whisper on an existing 1:1 session replaces the ratchet (`planSessionOpen`) unless the identity changed | Server can still **drop** members. Stolen **live** admin with no pending roster can still distribute **its** sender keys until revoke. Decline without revoke cannot tell other members. History already decrypted at an old epoch stays on the device. Failed SKDM share leaves that device blind until retry. Archived 1:1 ratchets older than 40 are dropped |
...
| DB leak | M | Metadata + ciphertext | Encryption at rest (platform), peppered phones, TTL; consumed OPK publics wiped; revoked device directory material wiped; ACK deletes mailbox rows; revoke wipes drafts and refresh hashes | Graph, sizes, times; live public keys remain (needed for X3DH); undelivered mail until TTL |
